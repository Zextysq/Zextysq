...
const rp = ['погладил', 'погладила', 'обнял', 'обняла', 'поцеловал', 'поцеловала', 'похвалил', 'похвалила', 'пнул', 'пнула', 'сжёг', 'сожгла', 'убил', 'убила', 'лизнул', 'лизнула', 'взорвал', 'взорвала', 'связал', 'связала', 'ударил', 'ударила', 'шлёпнул', 'шлёпнула', 'кусьнул', 'укусил', 'кусьнула', 'укусила', 'интиму', 'покормил', 'покормила'];
...
const mesArr24 = ['Ну хоть кто-то кормит нормально!', 'Спасибо!', 'Ням!'];
...
const mesArr24_bad = ['Я не собираюсь есть эту дрянь', 'Фу, ты отвратителен', 'Какая мерзость', '*Метнула в лицо*'];


const mesArr3_good = ['Слушаю', 'Что?', 'Что-то хочешь, ня?', 'Я здесь'];
const mesArr4_good = ['Привeтик!', 'Ох, привeтствую тебя!', 'О, а вот и ты)', 'Рада тебя видеть!'];
const mesArr5_good = ['Спoкойной!', 'Спи сладко :3', 'Спoкoйной ночи, ня!'];
const mesArr6_good = ['Дoбрoе утро чатик :3', 'Утрeца!', 'Дoбрoе утро!', 'Пожать руку'];

const mesArr7_good = ['Приятно...', '*Мурлычет*', 'Ня :3', 'Обнять'];
const mesArr8_good = ['Обнимашки!', 'Ня :3', 'Поцеловать'];
const mesArr9_good = ['*Растеряно посмотрела прямо в глаза*', '*Краснеет*', '&#128536;', 'А с языком слабо?'];
const mesArr9_2_good = ['М-мф...', 'Ого...', '&#128536;'];
...
const mesArr24_good = ['М-м... Вкусно!', 'Пасибки :3', 'Ням!', 'Да ты неплохо готовишь!'];
...
easyvk({
  ...
  function msgResearch(data) {
    ...
    if (type == true) {
      if ((text.indexOf('@all') > -1) || (text.indexOf('@online') > -1) && (text.indexOf('объявил общий сбор!') == -1)) MsgSend(['Что за сбор?', peer]);
    }
    ...
    if (reputation <= -25) {
      if (vkid == '-174105461') {
        ...
        if ((text.indexOf('покормил [' + userID) > -1) || (text.indexOf('покормила [' + userID) > -1)) MsgSend([mesArr24_bad[getRandomInt(4)], peer]);
      }
    ...
    if (reputation >= 25) {
      if (vkid == '-174105461') {
        ...
        if ((text.indexOf('поцеловал [' + userID) > -1) || (text.indexOf('поцеловала [' + userID) > -1)) {
          if (text.indexOf('язык') > -1) MsgSend([mesArr9_2_good[getRandomInt(3)], peer]);
          else MsgSend([mesArr9_good[getRandomInt(4)], peer]);
          repUp(parseInt(text.match(/\d+/)));
        }
        ...
        if ((text.indexOf('покормил [' + userID) > -1) || (text.indexOf('покормила [' + userID) > -1)) MsgSend([mesArr24_good[getRandomInt(4)], peer]);
      }
    ...
    if ((reputation > -25) && (reputation < 25)) {
      if (vkid == '-174105461') {
        ...
        if ((text.indexOf('покормил [' + userID) > -1) || (text.indexOf('покормила [' + userID) > -1)) MsgSend([mesArr24[getRandomInt(3)], peer]);
      }
