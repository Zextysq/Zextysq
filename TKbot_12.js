...
easyvk({
  ...
  function DialogTypeTest (data) {
    ...
    let reply = data['message']['reply_message'];
    let attachments = data['message']['attachments'];
    let from_id = data['message']['from_id'];

    vk.call('messages.getConversationsById', {
      peer_ids: peer
    }).then( async (data) => {
      if (data['items'][0]['peer']['type'] == 'user') MsgTest([text, peer, reply, attachments, true, from_id]);
      else MsgTest([text, peer, reply, attachments, false, from_id]);
    });
  }
  ...
  function kick(data) {
    ...
    MsgSend(['А нечего было Топор кидать! \n Жди пока тебя кто-нибудь вернёт', peer]);
    ...
    vk.call('messages.removeChatUser', {
      ...
      console.log('[X] Не удаётся исключить пользователя из беседы, ошибка!');
      console.log(error);
    });
  }
