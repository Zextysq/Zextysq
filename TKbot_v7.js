...
const mesArr6 = ['Не смешно &#128545;', 'Вам ещё не надоело?', 'ТАМ НАПИСАНО "МОЙ ПОЗВОНОЧНИК"!!!', 'БАН!'];
...
easyvk({
  ...
  LPB.connect({
    ...
    connection.on('message_new', (msg) => {
      ...
      if ((msgTxt.indexOf('Колумн') > -1) || (msgTxt.indexOf('колумн') > -1)) {

        vk.call('messages.send', {
          message: mesArr6[getRandomInt(4)], 
          token: botGroupToken,
          peer_id: msgPeerID,
          random_id: easyvk.randomId()
        }).catch(error => {
