...
const mesArr6 = ['Сегодня что, какой-то праздник?', 'Мы не так уж близко знакомы'];
...
easyvk({
  ...
  LPB.connect({
    ...
    connection.on('message_new', (msg) => {
      ...
      if ((msgTxt.indexOf('Колумнуть тебя') > -1) || (msgTxt.indexOf('колумнуть тебя') > -1) || (msgTxt.indexOf('тебя колумнуть') > -1) || (msgTxt.indexOf('Колумну тебя') > -1) || (msgTxt.indexOf('колумну тебя') > -1) || (msgTxt.indexOf('тебя колумну') > -1)) {

        vk.call('messages.send', {
          message: mesArr6[getRandomInt(2)], 
          token: botGroupToken,
          peer_id: msgPeerID,
          random_id: easyvk.randomId()
        }).catch(error => {
