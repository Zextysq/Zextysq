...
const mesArr6 = ['Колумни, я не против', 'Ну колумни', 'Колумни, я жду'];
...
easyvk({
  ...
  LPB.connect({
    ...
    connection.on('message_new', (msg) => {
      ...
      if ((msgTxt.indexOf('Колумнуть тебя') > -1) || (msgTxt.indexOf('колумнуть тебя') > -1) || (msgTxt.indexOf('Колумну тебя') > -1) || (msgTxt.indexOf('колумну тебя') > -1)) {

        vk.call('messages.send', {
          message: mesArr6[getRandomInt(3)], 
          token: botGroupToken,
          peer_id: msgPeerID,
          random_id: easyvk.randomId()
        }).catch(error => {
