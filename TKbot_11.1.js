...
easyvk({
  ...
  LPB.connect({
    ...
    connection.on('message_new', (msg) => {
      DialogTypeTest([msg['message']['text'], msg['message']['peer_id']]);
    });
  ...
  function DialogTypeTest (data) {
    ...
    vk.call('messages.getConversationsById', {
      ...
      if (data['items'][0]['peer']['type'] == 'user') MsgTest([text, peer, true]);
      else MsgTest([text, peer, false]);
    });
  }
  ...
  function MsgTest (data) {
    ...
    if ((text.toLowerCase().indexOf('cпокойной ночи') > -1) || (text.toLowerCase().indexOf('сладких снов') > -1) || (text.toLowerCase().indexOf('добрых снов') > -1)) MsgSend([mesArr4[getRandomInt(3)], peer]);
    if ((text.toLowerCase().indexOf('доброе утро') > -1) || (text.indexOf('утрец') > -1)) MsgSend([mesArr5[getRandomInt(3)], peer]);
